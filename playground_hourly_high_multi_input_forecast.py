...
x_test_latest_batch = np_scaled[-input_sequence_length:,:].reshape(1,input_sequence_length,len(FEATURES))
...
actualDf =yf.download(symbol, end=end_date_time,start=start_date_time ,interval=interval)
print(actualDf)
...
if np.round(MAE, 2) <= 200 and np.round(MAPE, 2) <= 0.9 and np.round(MDAPE, 2) <=  0.9:
  ...
  with open(filename, mode='w', newline='') as file:
    ...
    data = [
    [actualDf['High'].values.max(), Predicted_High, actualDf['High'].values.max()-Predicted_High]
    ]
